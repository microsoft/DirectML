...
def random_zoom_crop(img: tf.Tensor) -> tf.Tensor:
    ...
    crops = tf.image.crop_and_resize([img], boxes=boxes, box_indices=np.zeros(len(zoom_levels)), crop_size=(32, 32))
    return crops[tf.random.uniform(shape=[], minval=0, maxval=len(zoom_levels), dtype=tf.int32)]
